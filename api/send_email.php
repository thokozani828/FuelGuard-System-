            ...
            <div class='container'>
                ...
                <div class='content'>
                    ...
                    <div class='expiry-note'>
                        Note: This invitation link is unique to you and will expire in 24 hours.
                    </div>
                </div>
                <div class='footer'>
                    <p>&copy; 2025 FuelGuard Pro. All rights reserved.</p>
...
if ($action === 'sendInvitation') {
    ...
    if (!$to || !$full_name || !$register_link) {
        echo json_encode(['status' => 400, 'error' => 'Missing required fields: email, full_name, and register_link are required']);
        exit();
    }
    ...
    if ($result['success']) {
        echo json_encode(['status' => 200, 'message' => 'Invitation sent successfully to ' . $to]);
    } else {
        echo json_encode(['status' => 500, 'error' => $result['error']]);
    }
    
} elseif ($action === 'sendCredentials') {
    $to = $data['email'] ?? '';
    $full_name = $data['full_name'] ?? '';
